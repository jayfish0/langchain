...
class BaseAgentQLTool(BaseTool):  # type: ignore[override]
    ...
    def __init__(self, *args, **kwargs):
        ...
        if self.api_key:
            os.environ["AGENTQL_API_KEY"] = self.api_key

        if "AGENTQL_API_KEY" not in os.environ:
            raise ValueError("AgentQL API key must be provided")
    ...
    def from_browser(
        ...
        lazy_import_playwright_browsers()
        return cls(sync_browser=sync_browser, async_browser=async_browser, api_key=api_key)  # type: ignore[call-arg]
